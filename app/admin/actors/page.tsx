...
export default function ActorsPage() {
  ...
  const [editingActor, setEditingActor] =
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <h1 className="text-3xl font-bold">გახმოვანების მსახიობები</h1>
        ...
        <Select value={filter} onValueChange={(value: any) => setFilter(value)}>
          ...
          <SelectContent>
            <SelectItem value="all">
              ყველა მსახიობი ({actors.length})
            </SelectItem>
            <SelectItem value="active">
              აქტიური ({actors.filter((a) => a.is_active).length})
            </SelectItem>
            <SelectItem value="inactive">
              არააქტიური ({actors.filter((a) => !a.is_active).length})
            </SelectItem>
            <SelectItem value="featured">
              რჩეული ({actors.filter((a) => a.is_featured).length})
            </SelectItem>
          </SelectContent>
        </Select>
      </div>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        დარწმუნებული ხართ, რომ გსურთ {actor.name}-ის წაშლა? ეს
        ...
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-muted-foreground">
              {searchTerm
                ? "ძებნის შედეგები არ მოიძებნა"
                : "მსახიობები ჯერ არ დამატებულა"}
            </p>
          </CardContent>
        </Card>
      )}
